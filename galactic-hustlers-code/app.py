...
founder_wallet = (
    "FounderWalletPublicKey"  # Replace with actual founder wallet public key
)
...
class GalacticWorm:
    ...
    def race(self):
        ...
        if self.is_near_home():
            print(
                f"{self.name} has disappeared, as they are too close to their origin!"
            )
...
class GalacticHustler:
    ...
    def place_bet(
        self, sol_wallet: SolanaWalletConnect, gworm_wallet: SolanaWalletConnect
    ):
...
class GalacticRace:
    ...
    def start_game(self):
        ...
        if (
            len(self.galactic_worms) >= MIN_GALACTIC_WORMS
            and len(self.galactic_hustlers) >= MIN_GALACTIC_HUSTLERS
        ):
